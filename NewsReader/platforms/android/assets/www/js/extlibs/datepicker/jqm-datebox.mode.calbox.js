...
(function($) {
    ...
    $.extend($.mobile.datebox.prototype, {
        ...
        _cal_check: function(cal, year, month, date) {
            ...
            if (ret.ok) {
                ...
                var checkFoodLog = $("#foodLogCalendar").length;
                var checkActivityLog = $("#activityLogCalendar").length;
                var checkBsLog = $("#bsLogCalendar").length;
                
                var checkActivityRecord = $("#activityRecordsCalendar").length;
                var checkFoodRecord = $("#foodRecordsCalendar").length;
                var checkBsRecord= $("#bsRecordsCalendar").length;
                ...
                var dataKey = year + "-" + pad0Number(month + 1) + "-" + pad0Number(date);
                var dict;
                
                if (checkFoodLog === 1 || checkFoodRecord===1) {
                    dict = loadFoodLoadDate();
                    highLight=1;

                } else if (checkActivityLog === 1 || checkActivityRecord===1) {
                    dict = loadActivityLoadDate();
                    highLight=1;
                } else if (checkBsLog===1 || checkBsRecord===1){
                    dict=loadBsLoadDate();
                    highLight=1;
                }
                
                if(checkFoodRecord===1 || checkActivityRecord===1 || checkBsRecord){
                    record=1;
                }

                if (o.calHighPick && date === cal.presetDay && (w.d.input.val() !== "" | o.defaultValue !== false)) {
                    ret.theme = o.themeDatePick;
                } else if (o.calHighToday && ret.comp === cal.thisDate.comp()&& record!==1) {
                    ret.theme = "e";
                ...
                } else if (highLight === 1 && dict[dataKey] !== undefined) {
                    ret.theme = "c";
                } 
            }
    ...
    $.extend($.mobile.datebox.prototype._build, {
        'calbox': function() {
            ...
            $("<div style='border-style:none' class='" + uid + "gridplus" + (w.__('isRTL') ? '-rtl' : '') + "'><a href='#'>" + w.__('nextMonth') + "</a></div>")
            ...
            $("<div style='border-style:none;margin-left:10px' class='" + uid + "gridminus" + (w.__('isRTL') ? '-rtl' : '') + "'><a href='#'>" + w.__('prevMonth') + "</a></div>")
                    .prependTo(w.d.intHTML.find('.' + uid + 'gridheader'))
